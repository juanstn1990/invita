...
export function renderInvitation(opts: RenderOptions): string {
     ...
     una portada distinta. */
  {
    ...
    /* Las velas: cinco encendidas abajo y el resplandor que las envuelve al
       tocarlas. Cinco y no tres: con tres, la fila se lee como un adorno
       centrado; con cinco, como una mesa puesta. */
    if (velo && apertura === "velas") {
      velo.insertAdjacentHTML?.(
        "afterbegin",
        `<div class="inv-velas" aria-hidden="true">` +
          `<i class="inv-vela"></i><i class="inv-vela"></i><i class="inv-vela"></i>` +
          `<i class="inv-vela"></i><i class="inv-vela"></i></div>` +
          `<i class="inv-resplandor" aria-hidden="true"></i>`
      );
      velo.insertAdjacentHTML?.("beforeend", `<p class="inv-sobre-pista">Toca las velas</p>`);
    }
  ...
  if (document.querySelector("[data-inv-sobre],[data-inv-libro],[data-inv-abanico],[data-inv-agendar],[data-inv-wa],.inv-velo-deseo,.inv-velo-nubes,.inv-velo-telon,.inv-velo-anillos,.inv-velo-claqueta,.inv-velo-mariposa,.inv-velo-jardin,.inv-velo-velas,.inv-velo-escarcha,.inv-velo-naipe,.inv-cielo,.inv-rasca-capa")) {
